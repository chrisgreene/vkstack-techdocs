...
// Static hosting: answers are precomputed JSON bundles under /public (served at /).
window.VKSTACK_STATIC = true;

// Optional-layer / generation bundles may be missing from the repo (they are large).
// Prefer local files; fall back to the upstream static site so NSX/Avi/TMC still work.
const DATA_CDN = "https://vkstack.warroyo.com";
...
async function fetchBundle(name) {
  ...
  throw new Error(`could not load the site data (${name}: local ${local.status}, cdn ${remote.status})`);
}
...
function siteData(withKey = "", genKey = "") {
  ...
  if (!bundles.has(cacheKey)) {
    ...
    bundles.set(cacheKey, fetchBundle(name));
  }
